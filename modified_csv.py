...
def replace_paths_in_csv(file_path, output_file_path):
    ...
    new_path_2_new = '/data/public_datasets/MAFW/audio/'
    ...
    for row in lines:
        for i, cell in enumerate(row):
            if new_path_2 in cell:
                #print(cell)
                row[i] = cell.replace(old_path, new_path)
                cell = row[i]
                row[i] = cell.replace(new_path_2, new_path_2_new)
...
for i in range(1, 6, 1):
    for name in 'train','test':
        file_path = f'saved/data/mafw/audio_visual/single/split0{i}/{name}.csv'  # 输入文件路径
        output_file_path = f'saved/data/mafw/audio_visual/single/split0{i}/{name}.csv'  # 输出文件路径
